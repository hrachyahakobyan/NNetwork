...
public:
	...
	typedef std::pair<std::vector<double>, int> TrainInput;
	void train(std::vector<TrainInput>& trainData, std::size_t epochs, std::size_t batch_size, double eta);
...
private:
	...
	std::pair<std::vector<NNMatrix<double>>, std::vector<NNMatrix<double>>> back_prop(const std::vector<double>& X, int Y);
